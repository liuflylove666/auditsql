...
export default {
  ...
  methods: {
    ...
    putemail () {
      axios.put(`${util.auth}/users/userinfo/changemail/`, {
    ...
    Registered () {
      this.$refs['userinfova'].validate((valid) => {
        if (valid) {
          axios.post(util.auth + '/users/userinfo/', {
    ...
    refreshuser (vl = 1) {
        axios.get(`${util.auth}/users/userinfo/?page=${vl}`)
        .then(res => {
    ...
    saveEditPass () {
      this.$refs['editPasswordForm'].validate((valid) => {
        if (valid) {
          ...
          axios.put(util.auth + '/users/userinfo/changepwd/', {
              'username': this.username,
    ...
    saveEditInfo () {
      axios.put(util.auth + '/users/userinfo/changegroup/', {
          'username': this.username,
    ...
    delUser () {
      if (this.username === this.confirmuser) {
        axios.delete(util.auth + '/users/userinfo/' + this.username + '/')
          .then(res => {
            this.$Notice.success({
              title: '通知',
              desc: res.data
