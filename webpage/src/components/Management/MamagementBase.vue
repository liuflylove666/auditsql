    ...
    <div class="edittable-testauto-con">
      <Form ref="formValidate" :model="formItem" :label-width="100" :rules="ruleInline">
        ...
        <Form-item label="端口:" prop="port">
          <Input v-model="formItem.port" placeholder="请输入" number></Input>
        </Form-item>
  ...
  methods: {
    ...
    testlink () {
      this.$refs['formValidate'].validate((valid) => {
        if (valid) {
          axios.put(util.auth + '/managedb/databaseinfo/', {
              'ip_address': this.formItem.ip,
    ...
    add () {
      for (let i of this.rowdata) {
          // alert('aaa:' + i.conn_name);
        if (i.conn_name === this.formItem.name) {
          this.$Notice.error({
            title: '警告',
            desc: '连接名称重复,请更改为其他！'
          })
      ...
      this.$refs['formValidate'].validate((valid) => {
        if (valid) {
          ...
          axios.post(`${util.auth}/managedb/databaseinfo/`, {
    ...
    edit_tab (index) {
      this.$Modal.info({
        title: '数据库连接信息',
        content: `机房: ${this.rowdata[index].computer_room}<br> 连接名称：${this.rowdata[index].conn_name}<br>
                  数据库地址：${this.rowdata[index].ip_address}<br>端口: ${this.rowdata[index].port}<br>用户名: ${this.rowdata[index].conn_user}`
      })
    },
    // 删除数据库
    deldatabases (index) {
      this.delbaseModal = true
      this.delbasename = this.rowdata[index].conn_name
    },
    // 删除数据库字典
    Deletedic () {
    ...
    delbaselink () {
      if (this.delbasename === this.delconfirmbasename) {
        axios.delete(util.auth + '/managedb/databaseinfo/' + this.delbasename + '/')
          .then(res => {
            this.$Notice.success({
              title: '通知',
              desc: res.data
    ...
    mountdata (vl = 1) {
      axios.get(`${util.auth}/managedb/databaseinfo/?page=${vl}`)
        .then(res => {
